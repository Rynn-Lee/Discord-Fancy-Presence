...
const Label = styled.label<DataFancyComponentProps>`
  ...
  display: block;
  text-align: ${(props) =>
    props?.datafancy?.position ? props.datafancy.position : "center"};
  top: 0px;
  left: 0px;
  width: 100%;
  height: 100%;
...
const FancyInput = styled.input<DataFancyComponentProps>`
  ...
  &:focus {
    ...
    background: var(--var-accent1-hover);
    color: #ffffff;
  }
  &:hover {
    background: var(--var-accent1-hover);
  ...
  &:hover + .label {
    opacity: ${(props: any) =>
      props?.datafancy?.text && !props?.inputProps ? "0" : "1"};
  }
`;
...
const Input: React.FunctionComponent<InputProps> = ({
  ...
  return (
    <InputWrapper>
      <FancyInput {...inputProps} value={inputProps.value} datafancy={fancy} />
      {fancy ? (
        <Label
