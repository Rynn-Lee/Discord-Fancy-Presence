...
export default function Preview({ appInfo }: any) {
  ...
  useEffect(() => {
    checkImage(appInfo.largeImageKey).then((res: boolean) =>
  ...
  useEffect(() => {
    checkImage(appInfo.smallImageKey).then((res: boolean) =>
    ...
    <div className={styles.preview}>
      <fieldset className={styles.discordRPC}>
        ...
        <div className={styles.photos}>
          ...
          {appInfo.smallImageKey && appInfo.largeImageKey ? (
            <Image
      ...
      <Warnings styles={styles} isSquare={isSquare} appInfo={appInfo} />
    </div>
  );
}
